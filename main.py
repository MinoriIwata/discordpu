...
TOKEN = 'OTM1OTM3MTE3OTk2MDgxMzAz.YfF5XQ.cxQmRlUceMXnHQG5d3BoMJn67RY'
...
async def on_message(message):
    ...
    if message.content=="!RP" or message.content=="!rp":
        #↓
        RP = ["www","何してるんですか？","進捗ダメです","ねこはいますよろしくおねがいします","やめてもろて","あ！野生のPLが飛び出してきた！","今あなたの後ろにいるの","ピザのお届けです","では、1d20/1d100のSANcです","食らっちゃうよ～ん","ちくわ大明神","あなたはいまほんとうにただしいあなた？","わたしはわたし？それともあなた？","33-4","いまあなたのうしろにいるの","そしたら私がふりおろすから","お腹すきました","ドンペリ入りま～す","ご主人！お帰りにゃ！","お知らせです"+message.author.mention+"さんがお呼びでした","笹食ってる場合じゃねぇ","一体いつから俺が喋ると錯覚していた？","もうちっとだけ続くんじゃ"]
        choice = message.author.mention + random.choice(RP)  #←
        await message.channel.send(choice)

    if message.content == "!RPa" or message.content=="!rpa":
        #↓
        RP = ["「いってらっしゃい」「おかえりなさい」", "「おはよう」「おやすみ」", "「こっちにおいで」", "「ご飯にする？お風呂にする？それとも…」", 
        "「さよなら」","苦手な食べ物を前にして","お化け屋敷にて","「愛してるよ」","「さようなら」","初めましてじゃない「初めまして」","「大好きだよ」「大嫌いだよ」",
        "嘘がバレた時の一言","小さな嘘を、ひとつ","死亡フラグ、立ててもろて","焦った一言","関係性が深い相手が死んだときの一言","殺すときの一言","死ぬときの一言","復讐相手に一言","恨み言","大切な人が川の此方へ来た時の一言"]
        choice = message.author.mention + random.choice(RP)  #←
        await message.channel.send(choice)
    ...
    if message.content == "!RPe" or message.content=="!rpe":
        #↓
        RP = ["「いってらっしゃい」「おかえりなさい」", "「おはよう」「おやすみ」", "「こっちにおいで」", "「ご飯にする？お風呂にする？それとも…」", 
        "「さよなら」","苦手な食べ物を前にして","お化け屋敷にて"]
        choice = message.author.mention + random.choice(RP)  #←
        await message.channel.send(choice)

    if message.content == "!RPm" or message.content=="!rpm":
        #↓
        RP = ["「愛してるよ」","「さようなら」","初めましてじゃない「初めまして」","「大好きだよ」「大嫌いだよ」",
        "嘘がバレた時の一言","小さな嘘を、ひとつ","死亡フラグ、立ててもろて","焦った一言"]
        choice = message.author.mention + random.choice(RP)  #←
        await message.channel.send(choice)

    if message.content == "!RPd" or message.content=="!rpd":
        #↓
        RP = ["関係性が深い相手が死んだときの一言","殺すときの一言","死ぬときの一言","復讐相手に一言","恨み言","大切な人が川の此方へ来た時の一言"]
        choice = message.author.mention + random.choice(RP)  #←
        await message.channel.send(choice)
    ...
    if message.content.startswith('/ch_create '):
        ...
        permission = {
            ...
            addrole:discord.PermissionOverwrite(read_messages=True)
